<?php
   ...
   if(isset($_GET['cod']) && $_GET['cod'] !='')
    ...
    $_pdf->Cell(12, 3, 'ITENS POR  FORNECEDOR', 1, 1, 'C', FALSE);
   		...
   		foreach ($read_produto as $read_produto_view) 
        {
		    ...
		    if(mysqli_num_rows($read_item) > '0')
		    {
		        ...
		        foreach ($read_item as $read_item_view) 
				{
					...
					$_pdf->Cell( 2, 1, $read_item_view['cod_item'], 1, 0, 'C', TRUE);
		          	$_pdf->Cell( 2, 1, $read_item_view['valor_unitario'] , 1, 0, 'C', TRUE);
		          	$_pdf->Cell( 12, 1, $read_item_view['descricao'] , 1, 1, 'L', TRUE);                
